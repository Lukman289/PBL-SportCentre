  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 w-full h-full">
        <div className="absolute top-10 left-10 w-48 h-48 sm:w-72 sm:h-72 bg-primary/5 rounded-full blur-3xl"></div>
        <div className="absolute bottom-10 right-10 w-48 h-48 sm:w-72 sm:h-72 bg-primary/5 rounded-full blur-3xl"></div>
      </div>
      ...
      <div className="container mx-auto relative z-10 px-4 sm:px-6 md:px-8">
        <motion.div 
          ...
          variants={fadeInUp}
        >
          <h2 className="text-xl sm:text-4xl font-bold mb-2 sm:mb-4">Tingkatkan Performa Bisnis Anda</h2>
        ...
        <motion.div 
          className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-8 max-w-6xl mx-auto"
          variants={staggerContainer}
        >
          {stats.map((stat, index) => (
            <motion.div
              key={index}
              className={`relative group overflow-hidden bg-gradient-to-br ${stat.color} p-3 sm:p-8 rounded-xl shadow-sm hover:shadow-md transition-all border border-white/10`}
              variants={scaleIn}
              whileHover={{ 
                y: -5, 
            ...
            >
              ...
              <div className="flex flex-col h-full justify-between">
                <div>
                  ...
                  <h3 className="text-xl sm:text-5xl font-bold mb-0.5 sm:mb-2 bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent">
                    {stat.value}
                  </h3>
                  <p className="text-xs sm:text-lg font-medium">{stat.label}</p>
                </div>
                <p className="text-[10px] sm:text-sm text-muted-foreground mt-1 sm:mt-3">{stat.description}</p>
              </div>
              
              {/* Decorative elements */}
              <div className="absolute bottom-0 right-0 w-8 sm:w-16 h-8 sm:h-16 bg-white/5 rounded-tl-3xl"></div>
            </motion.div>
          ))}
        </motion.div>
      </div>
