      ...
      <div className="container mx-auto relative z-10">
        ...
        >
          {stats.map((stat, index) => (
            <motion.div
              key={index}
              className={`relative group overflow-hidden bg-gradient-to-br ${stat.color} p-8 rounded-2xl shadow-sm hover:shadow-md transition-all border border-white/10`}
              variants={scaleIn}
              whileHover={{ 
                y: -5, 
            ...
            >
              ...
              <div className="flex flex-col h-full justify-between">
                <div>
                  ...
                  <h3 className="text-5xl font-bold mb-2 bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent">
                    {stat.value}
                  </h3>
              ...
              {/* Decorative elements */}
              <div className="absolute bottom-0 right-0 w-16 h-16 bg-white/5 rounded-tl-3xl"></div>
            </motion.div>
          ))}
        </motion.div>
      </div>
